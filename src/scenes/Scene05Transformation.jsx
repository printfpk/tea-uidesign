...
export default function Scene05Transformation() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.apple-subtitle-5', 
        { opacity: 0, y: 30 },
        { opacity: 1, y: 0, duration: 1, ease: 'power2.out' },
        0.5
      )
      
      tl.to('.apple-subtitle-5', {
        opacity: 0,
        y: -30,
        duration: 0.5,
        ease: 'power2.in'
  ...
  return (
    <TransparentScrubScene ref={containerRef} className="scene-05" scrubDuration={2.5}>
        ...
        <div className="z-10 pointer-events-none text-center w-full">
          <h1 className="text-display text-display-xl leading-none tracking-tighter w-full">
            <span className="block pb-2 w-full text-center">
              <SplitText type="blur" className="text-gold-400" duration={1.2}>THE</SplitText>
            </span>
            <span className="block pb-2 w-full text-center">
              <SplitText type="blur" className="text-ivory" duration={1.2} delay={0.2}>APEX</SplitText>
            </span>
          </h1>
          <p className="apple-subtitle-5 mt-8 text-body text-center max-w-lg mx-auto text-lg text-parchment-200 pointer-events-none">
            Crafted. Not manufactured. The final transformation.
          </p>
        </div>
